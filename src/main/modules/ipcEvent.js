 ...
 * @LastEditTime: 2021-07-06 10:19:09
 */
...
export default function () {
  ...
  ipcMain.on('toggle-play', (event, params) => {
    ...
    global.mainWindow.webContents.send('toggle-play', { value })
  })

  ipcMain.on('play-song', (event, params) => {
    let value = params.value
    global.mainWindow.webContents.send('play-song', { value })
  })

  ipcMain.on('prev-play', (event, params) => {
    let value = params.value
    global.mainWindow.webContents.send('prev-play', { value })
  })

  ipcMain.on('next-play', (event, params) => {
    let value = params.value
    global.mainWindow.webContents.send('next-play', { value })
  })
